...
import rozihub.rozihubprofessionals.adapter.AllVenuAdapterTomorrow
import rozihub.rozihubprofessionals.adapter.AllVenueAdapterAll
...
class MyBookingFragment : BaseFragment() {
    ...
    private lateinit var mListBookingAll: List<All>
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        ...
        newDetailedbookingList(object : ApiResultDeatiledbooking, ListnerForNaviagtionItem {
            ...
            override fun resultNewBooking(result: ResponseFromServerDetailedBooking) {
                ...
                if(result.Result.Allrevenue==0){
                    view.tv_nobookings.text = "No Bookings  "
                    view.tv_nobookings.visibility = View.VISIBLE

                    //showDialogWithDismiss("No  Bookings  for tommorow")
                }
                else{
                    if(!result.Result.All.isEmpty()){
                        mListBookingAll=result.Result.All
                        val veneuadapter= AllVenueAdapterAll(result.Result.All,this)
