...
class TomorrowFragment : BaseFragment() {
    ...
    private lateinit var mListBookingToday: List<Tommorow>
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        ...
        newDetailedbookingList(object : ApiResultDeatiledbooking, ListnerForNaviagtionItem {
            ...
            override fun resultNewBooking(result: ResponseFromServerDetailedBooking) {
                ...
                if(result.Result.tommorwrevenue==0){
                    view.tv_nobookings.text = "No Bookings for tommorow "
                    view.tv_nobookings.visibility = View.VISIBLE

                    //showDialogWithDismiss("No  Bookings  for tommorow")
                }
                else{
                    if(!result.Result.tommorow.isEmpty()){
                        mListBookingToday=result.Result.tommorow
                        val veneuadapter= AllVenuAdapterTomorrow(result.Result.tommorow,this)
